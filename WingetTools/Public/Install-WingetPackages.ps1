function Install-WingetPackages {
    ...
    .DESCRIPTION
        ...
        Stops when C: drive usage reaches or exceeds the threshold or after N packages.
    #>
    [CmdletBinding()]
    param(
        [string]$DatabasePath = (Join-Path (Split-Path -Parent (Split-Path -Parent $PSScriptRoot)) 'unigetui\winget-database.json'),
        [string]$LogPath = (Join-Path (Split-Path -Parent (Split-Path -Parent $PSScriptRoot)) 'install-log.json'),
